...
class AndroidFtsFallbackMatrixContractTests(unittest.TestCase):
    ...
    def test_wrapper_contract_is_present(self):
        ...
        self.assertIn("failed_device_results =", self.script)
        self.assertIn("device_lock_posture =", self.script)
        self.assertIn("device_lock_used =", self.script)
        self.assertIn("adb_path =", self.script)
    ...
    def test_dry_run_writes_per_device_json_and_summary(self):
        ...
        try:
            ...
            self.assertIn("- failed_devices: none", summary_markdown)
            self.assertIn("- device_lock_posture: not_acquired_dry_run", summary_markdown)
            self.assertIn("- device_lock_used: false", summary_markdown)
            self.assertIn("- adb_path: ", summary_markdown)
            self.assertIn("Android\\Sdk\\platform-tools\\adb.exe", summary_markdown)
            ...
            self.assertEqual(first_device["device_lock_posture"], "not_acquired_dry_run")
            self.assertFalse(first_device["device_lock_used"])
            self.assertTrue(first_device["adb_path"].endswith("Android\\Sdk\\platform-tools\\adb.exe"))
