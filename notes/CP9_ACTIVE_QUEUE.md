...
- **(j) `scripts/*.ps1` non-AGENTS-named tracking follow-up.**
  ...
  - `scripts/start_fastembed_server.ps1` - DEFER to separate follow-up; adjacent to the overnight/start branch but not implicated in the D28 missing-loop dependency. Future work should still rerun Rule-2b secret-scan/HARD-STOP policy first, then evaluate cross-references in its own slice.
  - `scripts/start_litert_host_server.ps1` - DEFER to separate follow-up; adjacent to the overnight/start branch but not implicated in the D28 missing-loop dependency. Future work should still rerun Rule-2b secret-scan/HARD-STOP policy first, then evaluate cross-references in its own slice.
