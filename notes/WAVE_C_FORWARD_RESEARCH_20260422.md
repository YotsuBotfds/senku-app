...
Single emission per ask lifecycle at the terminal return site. Route strings (from the R-telemetry forward research §3, likely preserved in landed code):
...
- `topVectorSimilarity ∈ [0.45, 0.62]` → UNCERTAIN_FIT (lines 1270-1272)
- Other: CONFIDENT
...
| 5 | `topVectorSimilarity ∈ [0.45, 0.62]` | UNCERTAIN_FIT | `:1264-1266` |
...
**Rules 3-5 are the tuning surface.** The constants in §3 feed these.
...
- Known thin-but-real patterns flagged in ABSTAIN_TUNING_ANALYSIS_20260418 §Constant-by-Constant (e.g., "dirty nail", "clean brown water").
...
1. **Slice order.** Default: §6 ordering (W-C-0 → W-C-1 → W-C-2 → W-C-3 → W-C-4 → W-C-5 optional). Justify any reorder.
...
5. **Probe regression discipline.** Lock the "pre-tune / post-tune probe diff" as a hard step in W-C-2+ slices, not optional. Reference §11.5.
6. **Safety-invariant guard test coverage.** Audit before W-C-2 dispatch; add tests in-slice if missing (§11.4).
...
1. **Short planner direction note** (~30-80 lines) that locks:
   - Wave C slice order (defaults to §6 above but open to reorder)
