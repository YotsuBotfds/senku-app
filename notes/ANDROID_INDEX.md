...
- [`../scripts/run_android_route_smoke.ps1`](../scripts/run_android_route_smoke.ps1): live-safe current-head route proof that runs only `PackRepositoryCurrentHeadRouteSmokeAndroidTest`; keep broad route parity manual/heavy
- [`ANDROID_VALIDATION_LADDER.md`](./ANDROID_VALIDATION_LADDER.md): compact Android proof ladder; use focused tests per commit, checkpoint gates after small waves, live route smoke for quick route proof, and broad parity/full matrices only by explicit request
...
- [`../scripts/run_windows_validation.ps1`](../scripts/run_windows_validation.ps1) `-Mode android-migration` / [`../scripts/run_android_migration_validator_suite.ps1`](../scripts/run_android_migration_validator_suite.ps1): no-emulator Android migration validator batch; validates helper summary contracts only and is not UI acceptance evidence
...
- [`../scripts/run_android_uiautomator_24_comparison.ps1`](../scripts/run_android_uiautomator_24_comparison.ps1) / [`../scripts/validate_android_uiautomator_24_comparison_summary.py`](../scripts/validate_android_uiautomator_24_comparison_summary.py): dry-run-only UIAutomator 2.4 beta comparison scaffold; records planned dependency/test lane without changing Gradle dependencies, launching instrumentation, or creating UI acceptance evidence
...
- [`../scripts/run_android_followup_suite.ps1`](../scripts/run_android_followup_suite.ps1): follow-up suite replay wrapper, guarded by `tests/test_run_android_followup_suite_contract.py`
...
- [`../scripts/stop_android_harness_runs.ps1`](../scripts/stop_android_harness_runs.ps1): mixed host-process and device-package stop helper, guarded by `tests/test_stop_android_harness_runs_contract.py`
- [`../scripts/stop_android_harness_orphans.ps1`](../scripts/stop_android_harness_orphans.ps1): host-side stale Android prompt/follow-up harness-tree cleanup, guarded by `tests/test_stop_android_harness_orphans_contract.py`
...
- [`ANDROID_MIGRATION_BACKLOG_20260427.md`](./ANDROID_MIGRATION_BACKLOG_20260427.md): current-head Android migration blockers after matrix proof, including the compact migration-proof checklist for future Android lanes
